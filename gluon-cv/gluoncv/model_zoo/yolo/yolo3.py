...
from .darknet import _conv2d, darknet53, Tiny_darknet
...
class YOLOOutputV3(gluon.HybridBlock):
    ...
    def reset_class(self, classes, reuse_weights=None):
        ...
        self._num_pred = 1 + 4 + len(classes) + self._num_bases
        all_pred = self._num_pred * self._num_anchors
    ...
    def hybrid_forward(self, F, x, anchors, offsets):
        ...
        # reshape to (B, xx,4+2+num_bases) bbox, id+scores, coef_center, coef(2*deg+2)
        detections = F.reshape(detections.transpose(axes=(1, 0, 2, 3, 4)), (0, -1, 6+self._num_bases))
        return detections
...
class YOLOV3(gluon.HybridBlock):
    ...
    def __init__(self, stages, channels, anchors, strides, classes, alloc_size=(128, 128),
        ...
        if num_bases == -1:
            raise NotImplementedError(f"num_bases not specified! (value: {num_bases})")
        self._num_bases =num_bases
        if pos_iou_thresh >= 1:
    ...
    def reset_class(self, classes, reuse_weights=None):
        ...
        if self._pos_iou_thresh >= 1:
            self._target_generator = YOLOV3TargetMerger(len(classes), self._ignore_iou_thresh, self._num_bases)
...
class TinyYOLOV3(gluon.HybridBlock):
    """Tiny YOLO V3 detection network.
    network imitate the tencentyoutu implementation
    Parameters
    ----------
    stages : mxnet.gluon.HybridBlock
        Staged feature extraction blocks.
...
def yolo3_tiny_darknet_voc(pretrained_base=False, pretrained=False,
                        norm_layer=BatchNorm, norm_kwargs=None, **kwargs):
    """YOLO3 multi-scale with darknet53 base network on VOC dataset.
    Parameters
    ----------
    ...
    from ...data import VOCDetection
    pretrained_base = False if pretrained else pretrained_base
    base_net = Tiny_darknet()
    stages = [base_net.features[:11], base_net.features[11:]]
    anchors = [[10, 13, 16, 30, 33, 23], [30, 61, 62, 45, 59, 119], [116, 90, 156, 198, 373, 326]]
    strides = [16, 32]
    classes = VOCDetection.CLASSES
    return get_yolov3(
