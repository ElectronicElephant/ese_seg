...
def validate(net, val_data, ctx, eval_metric, polygon_metric, args):
    ...
    mx.nd.waitall()
    net.hybridize()
    for batch_num, batch in enumerate(tqdm(val_data)):
        ...
        for x, y in zip(data, label):
            ...
            gt_coefs.append(y.slice_axis(axis=-1, begin=4, end=4+50))
            gt_ids.append(y.slice_axis(axis=-1, begin=4+50, end=5+50))
            gt_difficults.append(y.slice_axis(axis=-1, begin=5+50, end=6+50) if y.shape[-1] > 5 else None)
...
def demo_val(net, val_data, eval_metric, polygon_metric, ctx, args):
    ...
    logger.info('[Epoch {}] Validation @ {}: \n{}'.format(0, eval_metric.iou_thresh, val_msg))
    polygonval_msg = '\n'.join(['{}={}'.format(k, v) for k, v in zip(polygonmap_name, polygonmean_ap)])
    logger.info('[Epoch {}] MaskValidationn @ {}: \n{}'.format(0, polygon_metric.iou_thresh, polygonval_msg))
